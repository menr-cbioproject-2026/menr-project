...
print("PANCREATIC (PAAD) - RASSF1 and APC\n")
for gene, filename in genes.items():
    ...
    print(f"Of those, predicted responsive to MENR: {responsive_count} ({responsive_count/len(heavily_silenced)*100 if len(heavily_silenced)>0 else 0:.1f}%)")
    print()
